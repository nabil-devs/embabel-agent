...
Utility AI also supports the simpler `Flow` interface for nesting without specifying output types.
...
If you try to return an `Flow` from an action when using GOAP, the raw object will be returned (not executed as a nested agent).
