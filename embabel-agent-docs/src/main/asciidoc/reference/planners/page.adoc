...
|Utility only (meta-annotated with @Agentic)
|===
...
NOTE: The `@Agent` annotation is meta-annotated with `@Agentic`, so any `@Agent` class can also be used as a nested agent with Utility AI.
...
Since `@Agent` is meta-annotated with `@Agentic`, any `@Agent` class can be returned from actions when using Utility AI.
