...
For GOAP planning, the nested class must have exactly one business goal (defined via `@AchievesGoal` or `@Action(goal="...")`) so the planner knows the output type.
...
2. The goal's output type (from `@AchievesGoal` or `@Action(goal="...")`) tells the planner what type the workflow produces
3. Within each workflow, the planner selects which action to execute based on preconditions and effects
4. When a goal-achieving action completes, the workflow finishes and returns its result
...
|`@Subflow` with no goal
|No - output type unknown
|Yes
...
TIP: For GOAP compatibility, ensure your nested class has exactly one goal defined via `@Action(goal="...")` or `@AchievesGoal`.
Utility AI is more flexible and works with any number of goals (including none).
...
====== @Subflow Chaining (Multi-Level Nesting)

@Subflow instances can return other @Subflow instances, enabling multi-level nested processing.
Each level must have a goal so the planner can resolve the output type chain.
...
....
┌─────────────────────────────────────────────────────────────────────────────┐
│                         TwoLevelNestingAgent                                │
│                                                                             │
│  UserInput ──► start() ──► Level1 ──► Level2 ──► Frog ──► end() ──► String │
│                              │          │         │                         │
│                              ▼          ▼         │                         │
│                          (nested)   (nested)      │                         │
│                           agent      agent        │                         │
...
Planning resolution (right to left):
  end() needs Frog ◄── Level2 produces Frog ◄── Level1 produces Level2 ◄── start()
....
...
class TwoLevelNestingAgent {
    ...
    @Action
    fun start(userInput: UserInput): Level1 {                   // <1>
        return Level1("Processing: ${userInput.content}")
    }

    @Subflow
    class Level1(val name: String) {

        @Action(goal = "Convert to Level2")                      // <2>
        fun toL2(): Level2 {
    ...
    class Level2(val person: PersonWithReverseTool) {
        ...
        @Action(goal = "Convert to Frog")                        // <3>
        fun toFrog(): Frog {
            return Frog(person.name)
        }
    }

    @Action(goal = "Complete with final message")                // <4>
...
<1> `start()` returns `Level1`, which is a `@Subflow`
<2> `Level1` has a goal that produces `Level2` (another `@Subflow`)
<3> `Level2` has a goal that produces `Frog` (final domain type)
<4> `end()` receives the `Frog` that bubbled up from the nested subflows

**How the planner resolves this:**

1. GOAP sees `end()` needs a `Frog`
2. `Level2.toFrog()` produces `Frog` → so `Level2` ultimately produces `Frog`
3. `Level1.toL2()` produces `Level2` → since `Level2` produces `Frog`, `Level1` ultimately produces `Frog`
4. `start()` returns `Level1` → so `start()` ultimately produces `Frog`
5. Plan: `start() → end()` (subflow execution is automatic)

IMPORTANT: Each `@Subflow` must define exactly one goal (via `@Action(goal="...")` or `@AchievesGoal`).
Without a goal, GOAP cannot determine what the subflow produces, and planning will fail.
