...
===== @Subflow: Simplified Nesting for Utility AI

When using the Utility planner, you can use a simpler nesting pattern with the `@Subflow` annotation.
Unlike `FlowReturning<O>`, `@Subflow` doesn't require specifying an output type--which makes sense because Utility AI doesn't need goal-oriented planning.
...
<1> Marker annotation - classes just need the annotation and `@Action` methods

====== When to Use @Subflow vs FlowReturning
...
|`@Agent` class
|Composing existing Spring-managed agents
|Utility only (meta-annotated with @Subflow)
|===

IMPORTANT: `@Subflow` nesting only works with the Utility planner.
If you try to return a `@Subflow` instance from an action when using GOAP, the raw object will be returned (not executed as a nested agent).

NOTE: The `@Agent` annotation is meta-annotated with `@Subflow`, so any `@Agent` class can also be used as a nested subflow with Utility AI.
...
Since `@Agent` is meta-annotated with `@Subflow`, any `@Agent` class can be returned from actions when using Utility AI.
...
IMPORTANT: `@Subflow` and `@Agent` class nesting only works with the Utility planner.
If you try to return a `@Subflow` or `@Agent` instance when using GOAP, the raw object will be returned (not executed as a nested agent).
