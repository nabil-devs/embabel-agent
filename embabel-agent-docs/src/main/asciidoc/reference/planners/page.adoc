...
Utility AI also supports the simpler `ActionClass` interface for nesting without specifying output types.
...
[[reference.planners__actionclass]]
===== ActionClass: Simplified Nesting for Utility AI

When using the Utility planner, you can use a simpler nesting pattern with `ActionClass`.
Unlike `Workflow<O>`, `ActionClass` doesn't require specifying an output type--which makes sense because Utility AI doesn't need goal-oriented planning.

[source,kotlin]
----
interface ActionClass  // <1>
----

<1> Marker interface with no methods - classes just need to implement it and have `@Action` methods

====== When to Use ActionClass vs Workflow

[cols="1,2,2",options="header"]
|===
|Interface |Use When |Planner Support
...
IMPORTANT: `ActionClass` nesting only works with the Utility planner.
If you try to return an `ActionClass` from an action when using GOAP, the raw object will be returned (not executed as a nested agent).

====== Example: ActionClass with Utility AI

[source,kotlin]
----
// Simple ActionClass - no output type needed
class ProcessingPhase(val data: String) : ActionClass {  // <1>
...
<1> `ActionClass` is simpler than `Workflow<O>` - no `outputType` property needed
<2> Still use `@AchievesGoal` to mark terminal actions
<3> Returning an `ActionClass` from an action enters it as a nested agent (Utility AI only)

====== ActionClass Chaining

ActionClass instances can return other ActionClass instances, enabling multi-phase processing:

[source,kotlin]
----
class ValidationPhase(val data: String) : ActionClass {
...
<1> Returning another `ActionClass` chains to the next phase

====== Dual Interface Classes

A class can implement both `ActionClass` and `Workflow<O>`.
In this case, it works with both planners - `Workflow<O>` behavior takes precedence:

[source,kotlin]
----
class DualPhase(val data: String) : ActionClass, Workflow<Result> {
